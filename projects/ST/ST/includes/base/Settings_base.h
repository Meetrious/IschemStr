...
namespace StraightTask
{
	...
	public:
		...
		void SaveError(float_t Tj, double_t current_error) {
	...
	public:
		...
		double_t GetRetValue(float_t tau, uint32_t Nj) {
			return data[CountRetIndex(tau, Nj)]; 
		}

		void ShiftRets(uint32_t Nj, double_t prev_U) { data[Nj - 1] = prev_U; }

		virtual void StateRetArray(uint32_t ST_N, float_t ST_t0, float_t ST_gap) {
	...
	private:
		[[nodiscard]] size_t CountRetIndex(float_t tau, size_t Nj) {
	...
	public:
		bool QueueRule(double_t& val, float_t tj, uint32_t gap_counter)
		{
	...
	public:
		...
		virtual void OutputBuds(uint32_t Nj, float_t Tj) = 0;
