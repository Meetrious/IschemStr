...
namespace StraightTask
{
	...
	public:
		...
		/**/Neurons::constants NEUR_C;

		Cytokines::Pro_Inflam::M_ODE CY;
		/**/Cytokines::constants CYTO_C;

		Adhesion::M_ODE ADH;
		/**/Adhesion::constants ADH_C;

		LeuMacrophags::M_ODE LM;
		/**/LeuMacrophags::constants LM_C;

		LeuNeutrophils::M_ODE LN;
		/**/LeuNeutrophils::constants LN_C;

		Microglia::Active::M_ODE MIA;
		Microglia::Inactive::M_ODE MII;
		/**/Microglia::constants MI_C;
		...
		/**/ToxDamage::constants D_C;

		Phagocytosis::Strong::M_Sub EPS_S;
		/**/Phagocytosis::constants EPS_C;
...
RETURN_TYPE ISolver<Method>::
	...
	T_Func(void)SetIniData(vector<double_t>& T0s) {
		...
		AS.SetInitialData(Mthd.X_init.tj, Mthd.X_init.ap_s);  T0s.emplace_back(Mthd.X_init.tj);
		AE.SetInitialData(Mthd.X_init.tj, Mthd.X_init.ap_e);  T0s.emplace_back(Mthd.X_init.tj);
	...
	T_Func(void)SetIniDataFromOutside(vector<double_t>& T0s) {
		...
		AS.SetInitialDataFromOutside(Mthd.X_init.tj, Mthd.X_init.ap_s); T0s.emplace_back(Mthd.X_init.tj);
		AE.SetInitialDataFromOutside(Mthd.X_init.tj, Mthd.X_init.ap_e); T0s.emplace_back(Mthd.X_init.tj);
	...
	T_Func(void)SynchronizeAllCoefs() {

		NEC.RP.SynchronizeCoefs(NEUR_C);
		AS.RP.SynchronizeCoefs(NEUR_C);
		AE.RP.SynchronizeCoefs(NEUR_C);
		HEL.RP.SynchronizeCoefs(NEUR_C);

		CY.RP.SynchronizeCoefs(CYTO_C);
		ADH.RP.SynchronizeCoefs(ADH_C);
		LM.RP.SynchronizeCoefs(LM_C);
		LN.RP.SynchronizeCoefs(LN_C);

		MIA.RP.SynchronizeCoefs(MI_C);
		MII.RP.SynchronizeCoefs(MI_C);

		DF.RP.SynchronizeCoefs(D_C);
		D_INI.RP.SynchronizeCoefs(D_C);
		EPS_S.RP.SynchronizeCoefs(EPS_C);
	}
