...
RETURN_TYPE ISolver<Method>::
	...
	T_Func(void)OutputBudgets(uint32_t Nj, double_t Tj) {
		...
		DP_A.OutputBuds(Nj, Tj);
		DP_N.OutputBuds(Nj, Tj);
	...
	T_Func(void)SynchronizeAllCoefs() {
		...
		DP_N.RP.SynchronizeCoefs(D_C);
		DP_A.RP.SynchronizeCoefs(D_C);
		EPS_S.RP.SynchronizeCoefs(EPS_C);
		EPS_W.RP.SynchronizeCoefs(EPS_C);
	}
